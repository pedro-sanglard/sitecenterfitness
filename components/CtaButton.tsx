...
import { CTA_LABEL, whatsappUrl } from "@/lib/config";
import { trackConversion } from "@/lib/analytics";

/**
 * Botao CTA padrao: abre o WhatsApp com a mensagem pre-preenchida e dispara o
 * evento de conversao (GA4 + Meta Pixel). Reusado em todas as secoes.
 */
export default function CtaButton({
  ...
  return (
    <a
      ...
      onClick={() => trackConversion(source)}
      aria-label={`${label} pelo WhatsApp`}
      className={`btn-cta ${fullWidthMobile ? "w-full sm:w-auto" : ""} ${className}`}
    >
      {/* Icone do WhatsApp */}
      <svg
